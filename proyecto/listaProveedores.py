...
class listaProveedores(Gtk.Window):
    """Ventana listaProveedores de la Casa del Libro.
                Metodos:
        ...
        if puntero is not None:
            ...
            if (nombre=="" or CIF=="" or direccion=="" or telefono=="" or correo==""):
                ...
                try:
                    baseDatos = dbapi2.connect("BaseDeDatos.dat")
                    cursor = baseDatos.cursor()
                    cursor.execute("UPDATE proveedores SET nombre = '"+nombre+"', CIF = '"+CIF+"', direccion='"+direccion+"', telefono='"+telefono+"', correo='"+correo+"'  WHERE id = '"+idProv+"'")
